...
def process_video(video_path, model_path1, model_path2, output_path):
    ...
    out = cv.VideoWriter(output_path + os.path.splitext(os.path.basename(video_path))[0] +'_第二首歌_' + str(timestamp) + '_predict.mp4', fourcc, fps, (width,  height))  # Create an empty video
    out2 = cv.VideoWriter(output_path + os.path.splitext(os.path.basename(video_path))[0] + '_第二首歌_' + str(timestamp) + '_person.mp4', fourcc, fps, (width,  height))  # Create an empty video
    ...
    with open(csv_file_path, mode='w', newline='') as file:
        ...
        while True:
            ...
            detect_params = model.predict(frame, device=0,classes=[0,1,2,4,5,8])
...
def main():
    # 使用範例
    video_path = "dance_video/20240306/testcam4_20240306110154.mp4"
