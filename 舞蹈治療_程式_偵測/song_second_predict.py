...
model = YOLO("model/pose_20240321_second.pt")
...
out = cv.VideoWriter("result_second_song/"+str(timestamp)+'_predict.mp4', fourcc, 20.0, (width,  height))  # Create an empty video
out2 = cv.VideoWriter("result_second_song/"+str(timestamp)+'_person.mp4', fourcc, 20.0, (width,  height))  # Create an empty video
csv_file_path = 'result_second_song/第二首歌_'+str(timestamp)+'_results.csv'
with open(csv_file_path, mode='w', newline='') as file:
    ...
    while True:
        ...
        out2.write(frame2)
        cv.imshow("second song",annotated_frame)
        cv.imshow("second song person", frame2) # Add this line to show boxes2 in a separate window
